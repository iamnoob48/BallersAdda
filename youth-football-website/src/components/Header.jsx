  ...
  return (
    <nav
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${
        scrolled ? "bg-white/80 shadow-md backdrop-blur" : "bg-transparent"
      }`}
    >
      <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <ul className="hidden md:flex gap-8 text-gray-700 font-medium">
          {["Tournaments", "Academies", "Login", "Register"].map((item) => (
            <li
              key={item}
        ...
        <button className="px-4 py-2 bg-green-600 text-white rounded-full font-semibold shadow-md hover:shadow-green-500/60 transition hover:scale-105 duration-300 glow-button">
          Host Tournament
        </button>
      </div>
    </nav>
